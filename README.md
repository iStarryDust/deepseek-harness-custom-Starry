...
| `packages/client/ui-conversation` | read-only transcript | **New: edit & regenerate** — every sent user message gains an inline edit action: reword the prompt, keep / remove / add images (png / jpeg / webp / gif), then resend. The conversation **forks at the edited message** — the original session is preserved and the turns after it are not carried over. The fork carries over the session's model selection; a failed resend keeps the current view and cleans up the leftover fork shell automatically |
