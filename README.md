...
| `packages/client/ui-conversation` | read-only transcript | **New: edit & regenerate** — every sent user message gains an inline edit action: reword the prompt, keep / remove / add images (png / jpeg / webp / gif), then resend. The conversation **forks at the edited message**: the original session is preserved, the turns after it are not carried over, and the fork inherits the session's model selection. A failed resend keeps the current view and removes the leftover fork shell; messages whose turns have left the loaded window after context compaction are no longer editable |
