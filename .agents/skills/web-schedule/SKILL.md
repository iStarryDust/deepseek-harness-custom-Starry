...
build and how the **Settings → General → 定时提醒** switch governs the feature. It
...
- **`@deepseek-ai/dsh-schedule-toggle`** — the switch plugin. It registers a
  "定时提醒 (Schedule reminders)" toggle in **Settings → General**. Enabling the
...
- **Session scope of tool registration.** The Schedule tools register for root
  agents active when the Schedule plugin is loaded. When the user reports
  `unknown tool "schedule_list"`, the plugin is not loaded yet: enable the toggle
  (which hot-reloads the plugin) and confirm the tools appear in the session —
  no new conversation is required.
- **Toggling and hot reload.** Toggling the switch reloads the configuration
  without a restart; the current session acquires the tools on load.
