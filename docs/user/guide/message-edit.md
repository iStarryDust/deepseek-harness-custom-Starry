...
1. Hover a sent user message and choose the pencil action. Editing is unavailable while the agent is still running a turn.
2. The inline editor opens with the original text focused. Below it, a strip shows the images the message carries.
...
- The original session is preserved in the sidebar; the branch appears as a new session. Neither is removed automatically — clean up the session list by hand when a branch is no longer needed.
- The branch carries over the model the original session used, so resending a message with images does not fall back to a text-only default model.
...
The view stays on the current session, and the editor reopens with your text and a failure notice. The leftover empty branch is removed automatically, so a failed edit leaves no stray sessions behind.
