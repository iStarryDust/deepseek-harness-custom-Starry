...
`agentPreset.capabilities`、`composeEnvironment` 与 `suggestEnvironment` 实现 per-agent 的按需环境（「定义模式」）。`capabilities` 列出浏览器所渲染的可切换能力清单。`composeEnvironment` 铸造一个一次性合并预设（`<agentId>-env-<slug>`），其组装是 `standard` 能力集的严格子集——调用方只选择要**保留**哪些能力，绝不指名插件，因此组装环境不会授予 `standard` 未携带的任何能力；返回的 id 随后经 `select` 绑定到空白会话。`suggestEnvironment` 用会话自身的模型路由对描述做一次一次性补全，推断所需能力 id；模型不可用时回退到确定性关键词分析。建议与部署默认能力合并，最终仍以使用者的勾选为准。
