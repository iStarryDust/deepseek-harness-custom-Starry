...
/**
 * The two conversation-adjacent surfaces: the new-session chip naming the next
 * session's preset, and the session header's read-only label. The split is the
 * host's rule — a session's history is produced under its preset's tools, so
 * the choice is only ever offered before one starts.
 */
...
const ROW_READY: AgentPresetSettingsState = {
  ...
  // `mine` deliberately names itself nothing: the label must fall back to the
