...
The `@deepseek-ai/dsh-schedule-toggle` package provides a **"定时提醒 (Schedule
...
- **Session-scoped tools.** The Schedule tools register for root agents active
  when the Schedule plugin loads. Enabling the switch hot-reloads the plugin, so
  the current session exposes the `schedule_*` tools without needing a new
  session.
